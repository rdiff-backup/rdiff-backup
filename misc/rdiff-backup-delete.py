...
import gzip, argparse, os, shutil, re, glob

parser = argparse.ArgumentParser(description="Delete files or directories from rdiff-backup mirrors including metadata", epilog='')
parser.add_argument('base', help='base directory of the backup (the one containing the rdiff-backup-data directory)')
parser.add_argument('path', help='path of the file or directory to delete, relative to the base directory')
parser.add_argument('-d','--dry', action='store_true', help='dry run - do not perform any action but print what would be done')
...
def check_type(base,path):
    """Determine whether specified path is a single file/link or directory that needs to be deleted recursively"""
    filetype = None
    p = base+'/'+path
...
def delete_entry(base,path):
    """Delete specified path, recursively if it is a directory.  Fall through if it doesn't exist (ie if it was removed from the source or mirror prior to script execution)"""
