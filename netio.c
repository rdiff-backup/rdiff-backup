	      ...
	      rs_write_fn_t write_fn, void *write_priv)
{
  ...
  if (length > INT32_MAX)
    {
      _hs_error ("length %u is too big", length);
      return -1;
    }
...

int
_hs_write_netlong (rs_write_fn_t write_fn,
		   void *write_priv,
		   uint32_t out)
{
  out = htonl (out);
...
int
_hs_write_netshort (rs_write_fn_t write_fn,
		    void *write_priv,
		    uint16_t out)
{
  out = htons (out);
...
int
_hs_write_netbyte (rs_write_fn_t write_fn,
		   void *write_priv,
		   uint8_t out)
{
  return write_fn (write_priv, (void *) &out, sizeof out) == sizeof out
...
int
_hs_read_netlong (rs_read_fn_t read_fn, void *read_priv, uint32_t * result)
{
  uint32_t buf;
  int ret;
