...
ADOC_LINKS = re.compile(
    r'(?:\W|^)(?:(?:link:|image::?|include::|xref:)([^[]+)\[|((?:ht|f)tps?://[^[]*)\[|link="?([^]"]+)"?\])'
)
...
def parse_args(args: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check URLs in asciidoctor files")
    parser.add_argument(
        "--verbose",
        "-v",
        action="store_const",
    ...
    parser.add_argument(
        "--timeout", "-t", type=int, default=2, help="Timeout in seconds (default 2)"
    )
    parser.add_argument(
        "adoc",
