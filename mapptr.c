...
_hs_map_ptr(hs_map_t * map, hs_off_t offset, int *len, int *reached_eof)
{
    ...
    *reached_eof = 0;
	...
	if (nread < 0) {
	    _hs_error("read error in %s: %s", __FUNCTION__, strerror(errno));
	...
	if (nread == 0) {
	     ...
	     * This is not considered an error.  If you keep * calling `read' 
	     * while at end-of-file, it will keep * returning zero and doing
	     * nothing else.  */
	    *reached_eof = 1;
	    break;
	}
