/* -*- c-file-style: "bsd" -*- * * $Id: mapptr.c,v 1.21 2000/06/01 03:30:38
   mbp Exp $ * * Copyright (C) 2000 by Martin Pool * Copyright (C) 1998 by
   Andrew Tridgell * * This program is free software; you can redistribute
...
/* TODO: Test this through a unix-domain socket and see what happens. * *
   TODO: Optionally debug this by simulating short reads. * * TODO: Make the
   default buffer smaller and make sure we test what * happens when it grows. 
 */

/* The Unix98 pread(2) function is pretty interesting: it reads data at a
   given offset, but without moving the file offset and in only a single
   call.  Cute, but probably pointless in this application. */
...
/* Set up a new file mapping.
   ...
   The file cursor is assumed to be at position 0 when this is called. For
   nonseekable files this is arbitrary; for seekable files bad things will
   happen if that's not true and we later have to seek. */
hs_map_t       *
_hs_map_file(int fd)
{
    hs_map_t       *map;
		...
		int *reached_eof)
{
	...
	if (nread < 0 && errno == EWOULDBLOCK) {
	    ...
	    /* Should we return null here?  We ought to tell the caller about 
	       this somehow, but at the same time we don't want to discard
	       the data we have already received. */
	    break;
	} else if (nread == 0) {
	    /* GNU libc manual: A value of zero indicates end-of-file *
	       (except if the value of the SIZE argument is also * zero).
	       This is not considered an error.  If you keep * *calling
	       `read' while at end-of-file, it will keep * returning zero and 
	       doing nothing else.  */
	    *reached_eof = 1;
	    break;
	}
...
/* Return a pointer to a mapped region of a file, of at least LEN bytes.  You 
   ...
   requested.  In this case, LEN will be reduced, and REACHED_EOF will be
   set.

   LEN may be increased if more data than you requested is available.

   The buffer is only valid until the next call to _hs_map_ptr on this map,
   or until _hs_unmap_file.  You certainly MUST NOT free the buffer.

   Iff an error occurs, returns NULL. */
/* @null@ */ const char *
_hs_map_ptr(hs_map_t * map, hs_off_t offset, ssize_t * len, int *reached_eof)
{
    /* window_{start,size} define the part of the file that will in the
       future be covered by the map buffer, if we have our way.

       read_{start,size} describes the region of the file that we want to
       read; we'll put it into the buffer starting at &p[read_offset]. */
    ...
    *reached_eof = 0;
    ...
    _hs_trace("map=<%p>, off=%ld, len=%ld", map, (long) offset, (long) *len);
	...
	*len = map->p_len - (offset - map->p_offset);
	...
	/* On some systems, it's much faster to do reads aligned with *
	   filesystem blocks.  This isn't the case on Linux, which has * a
	   pretty efficient filesystem and kernel/app interface, but * we
	   don't lose a lot by checking this. */
    ...
    /* read_min_size may be >*len when offset > map->p_offset, i.e. we have
       to read in some data before the stuff the caller wants to see.  We
       read it anyhow to avoid seeking (in the case of a pipe) or because
       they might want to go back and see it later (in a file). */
