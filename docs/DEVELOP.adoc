...
While this document focusses more on the processual and technical aspects of development (under Linux), it has a few companion documents highlighting specific aspects of development:

* xref:CODING.adoc[Coding Conventions].
* xref:DOCUMENTATION.adoc[Documentation Guidelines].
* And development guides addressing xref:Windows-DEVELOP.adoc[Windows] and xref:MacOS-DEVELOP.adoc[MacOS].

But let's start with this document.

toc::[]

== BEGINNER'S STEP BY STEP INTRODUCTION
...
* You have a https://github.com/join[GitHub account] we'll call `MYGHUSER` with an https://docs.github.com/en/authentication/connecting-to-github-with-ssh[SSH key assigned].
...
. https://github.com/rdiff-backup/rdiff-backup/fork[Fork the rdiff-backup repository] into your user `MYGHUSER`.
...
----
<1> This command must be called as the user with which you'll test. 
    Don't call it directly as root.
<2> This create a https://docs.python.org/3/tutorial/venv.html[Virtual Environment] so that you keep your system clean.
<3> It's really a single point followed by `.venv/...`.
...
<4> https://docs.opendev.org/opendev/bindep/[bindep] allows us to define non-Python dependencies in `bindep.txt`.
<5> If pip tells you to do so, it's just annoying.
<6> If using vim from within the venv gives you powerline warnings; again annoying...
    You may ignore potential deprecation warnings.
<7> This might fail under the more exotic Linux versions, feel free to provide a patch to `bindep.txt` or raise an issue together with the output of `bindep --profiles`.
<8> You're installing `rdiff-backup` from the Git repository.
<9> This should point at the `rdiff-backup` installed within the venv `.../.venv/bin/rdiff-backup`.
<10> The version is typically longer than usual and with a `dev` in it.
...
TIP: you can leave the venv with `deactivate`, then the `(.venv)` prefix disappears.
...
. There are many ways to test it, but the minimal set is `tox -e lint` followed by `tox -e py`.
+
IMPORTANT: Please don't submit code which hasn't passed both tests!
Also read the rest of this document and its companions in their entirety and comply with our guidlines and conventions.
If you encounter issues or have doubts, ask on the mailto:rdiff-backup-users@nongnu.org[mailing list] after https://lists.nongnu.org/mailman/listinfo/rdiff-backup-users[having registered].

. Once successful, you're ready for your first https://github.blog/developer-skills/github/beginners-guide-to-github-creating-a-pull-request/[Pull Request (PR)].

NOTE: The above instructions have been cursorily tested in containers using `podman` with the images `opensuse/tumbleweed`, `debian`, `ubuntu`, `archlinux`, `manjarolinux/base`, `quay.io/centos/centos:stream10`.
...
** .github/workflows/{deploy,weekly}.yml - check for `WIN_PYTHON_VERSION`
** tools/windows/group_vars/windows_hosts/generic.yml - check for `python_version` and `python_version_full` 
* Linux:
** tox.ini, tox_root.ini and tox_dist.ini - check for `envlist`
** .github/workflows/test_linux.yml - check for `python-version`
** .github/workflows/{deploy,weekly}.yml - check for `/opt/python/cp3...` (and possibly `many-linux`)
...
----
COVERAGE_FILE=.tox/pyXY/tmp/coverage.sqlite .tox/pyXY/bin/coverage report -m
----
