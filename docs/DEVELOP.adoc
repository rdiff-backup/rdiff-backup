...
sudo dnf debuginfo-install python3-debug
sudo dnf debuginfo-install --exclude "*.i686" \  #<1>
	bzip2-libs glibc librsync libxcrypt openssl-libs popt \
	sssd-client xz-libs zlib
----
...
[source,console]
----
$ CFLAGS='-Wall -O0 -g' pip --python $(which python3-debug) install .  # to be verified!!!
...
[source,console]
----
$ PYTHONTRACEMALLOC=1 gdb -args python3-debug -m rdiffbackup.run backup /some/source/dir /some/target/dir
(gdb) break src/_librsyncmodule.c:_librsync_new_patchmaker
(gdb) run
----
