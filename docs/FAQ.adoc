...
If you've backed-up a file or directory you shouldn't have backed-up, you can remove it again using `rdiff-backup remove file <repo>/<file-or-dir>` (after version 2.2.x) resp. `rdiff-backup-delete <repo>/<file-or-dir>` (at and before 2.2.x);
...
For earlier versions (before version 2.2), have a look at https://www.timedicer.co.uk/programs/help/rdiff-backup-regress.sh.php[rdiff-backup-regress] which completely removes the last made backup, but beware that regression takes a long time.
