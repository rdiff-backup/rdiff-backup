...
/* TODO: The common case is that the next block in both streams match. Can we 
   ...
   between blocks in the sum_struct corresponding to the order they're found
   in the input; then before doing a search we can just check that pointer. */
...
/* Return the positive token number for a match of the current block, or -1
   if there is none.

   If we don't find a match on the weak checksum, then we just give up.  If
   we do find a weak match, then we proceed to calculate the strong checksum
		 ...
		 hs_sumset_t const *sums, hs_stats_t * stats)
{
	...
	_hs_trace("found weak match for %#010x in token %d", weak_sum, token);
