...
=== RHEL 9, 10 and their replicas (from EPEL, x86_64)

----
sudo dnf install epel-release
sudo dnf install rdiff-backup
sudo dnf install python3-xattr  #<1>
----
<1> `python3-pyxattr` isn't available from EPEL 10, check the pip method if you need support for extended attributes.
...
=== CentOS (Stream) and RHEL 9 & 10 (from PyPi) - x86_64
...
sudo pip install rdiff-backup[meta]
----
...
See https://copr.fedorainfracloud.org/coprs/frankcrawford/rdiff-backup/[Frank's rdiff-backup COPR repo] to see which versions are currently supported, then follow the fitting instructions.
