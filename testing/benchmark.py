...
def benchmark(backup_cmd, restore_cmd, desc, update_func=None):
    ...
    comtst.re_init_subdir(TEST_BASE_DIR, b"rest_out")
...
def many(backup, restore, many_count=MANY_COUNT):
    ...
    manyout_dir = comtst.re_init_subdir(TEST_BASE_DIR, b"many_out")
    backout_dir = comtst.re_init_subdir(TEST_BASE_DIR, b"back_out")
    restout_dir = comtst.re_init_subdir(TEST_BASE_DIR, b"rest_out")
    ...
    return benchmark(
        backup_cmd,
        restore_cmd,
        "{count} 1-byte files".format(count=many_count),
        update_func,
    )
...
def nested(backup, restore, nested_depth=NESTED_DEPTH, nested_factor=NESTED_FACTOR):
    ...
    nestedout_dir = comtst.re_init_subdir(TEST_BASE_DIR, b"nested_out")
    backout_dir = comtst.re_init_subdir(TEST_BASE_DIR, b"back_out")
    restout_dir = comtst.re_init_subdir(TEST_BASE_DIR, b"rest_out")
    ...
    return benchmark(
        backup_cmd,
        restore_cmd,
        "{count} 1-byte nested files".format(count=nested_count),
        update_func,
    )
...
def print_results(bench, results):
    ...
    for ires in range(len(bench)):
        sys.stdout.write(
            "{name:{width}};".format(name=func_names[ires], width=names_width)
        )
...
print(
    "=== Results of '{bench}' benchmark (absolute and relative) ===".format(
