...
static int
_hs_check_gd_header (rs_read_fn_t ltread_fn, void *ltread_priv)
{
  int ret;
  ...
  ret = _hs_read_netlong (ltread_fn, ltread_priv, &remote_magic);
  return_val_if_fail (ret == 4, -1);
  if (remote_magic != expect)
    {
      _hs_error ("version mismatch: %#08x != %#08x",
	       remote_magic, expect);
      errno = EBADMSG;
      return -1;
    }
		...
		rs_read_fn_t read_fn, void *read_priv)
{
  ...
  if (type == op_copy_short_byte
      || type == op_copy_short_short
      || type == op_copy_short_int)
    {
    ...
    {
      /* must be an int */
      if (_hs_read_netlong (read_fn, read_priv, &tmp32) != sizeof tmp32)
	return -1;
      *offset = tmp32;
    }
		   ...
		   rs_read_fn_t read_fn, void *read_priv)
{
  ...
  if (cmd == op_literal_int)
    {
      ret = _hs_read_netlong (read_fn, read_priv, length);
    }
	   ...
	   hs_stats_t * stats)
{
  ...
  while ((ret = ltread_fn (ltread_priv, &type, 1)) == 1)
    {
      if (_hs_is_gd_eof (type) > 0)
	{
	  break;		/* We're done! Cool bananas */
	}
      else if (_hs_is_gd_literal (type, &length, ltread_fn, ltread_priv) > 0)
	{
	  ret = _hs_copy (length,
      ...
      else if (_hs_is_gd_signature (type, &length, ltread_fn, ltread_priv) > 0)
	{
	  ret = _hs_copy (length,
			  ltread_fn, ltread_priv,
	...
	{
	  _hs_error (__FUNCTION__ ": unexpected command %d!",
		   type);
