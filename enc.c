 ...
 * needs the old value.  
 * 
 * Fuck!  Software patents are *such* a stupid idea.  It's really a
 * bit simpler to just do the signature on the client, but we can make
 * do with this.  */
	  ...
	  int new_block_len, hs_stats_t * stats)
{
    ...
    int old_block_len, this_block_len;
    ...
    _hs_alloc_inbuf(inbuf, old_block_len);
	...
	while (at_eof
	       ...
	       : inbuf->cursor + old_block_len <= inbuf->amount) {
	    this_block_len = MIN(old_block_len, inbuf->amount - inbuf->cursor);
		...
		_hs_roll_sums(inbuf, rollsum, old_block_len);
		inbuf->cursor++;
	    }
	}
